...
def optimal_weights(n_points, er, cov):
    """
    Generats a list of weights to run the optmizer on to minimize the volatility
    """
...
def show_cppi(n_scenarios=50, mu=0.07, sigma=0.15, m=3, floor=0., riskfree_rate=0.03, steps_per_year=12, y_max=100):
    ...
    wealth_ax.set_xlim(left=0, right=steps_per_year*10 -1)
    ...
    if (floor > 0.01):
        ...
        hist_ax.annotate(f"Violations: {n_failures} ({p_fail*100:2.2f}%)\nE(shortfall)=${e_shortfall:2.2f}", xy=(.7, .7), 
